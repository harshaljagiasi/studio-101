...
export default function BookingPage() {
  ...
  const executeLiveBooking = async () => {
    // Requires email to proceed
    if (!selectedDate || selectedTimes.length === 0 || !purpose || !clientName || !clientPhone || !clientEmail) {
      alert('Please complete all form fields (including email) to process payment and confirm booking.');
      return;
    }
    ...
    setCustomPurpose(""); setSelectedEquipment([]); 
